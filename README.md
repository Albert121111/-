...
### Удобный вариант без локальной установки
Можно запускать проект в **GitHub Codespaces** (кнопка `Code` → `Codespaces` → `Create codespace`).
Тогда ничего не нужно скачивать на ПК, и можно просто выполнить:
```bash
npm run up
```

### CI в GitHub
Добавлен workflow `.github/workflows/ci.yml`: при `push` и `pull_request` автоматически запускаются тесты и базовые проверки.
Это не "хостинг" приложения, но помогает постоянно проверять, что проект не сломан.
